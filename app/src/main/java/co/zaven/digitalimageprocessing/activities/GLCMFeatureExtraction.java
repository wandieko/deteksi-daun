...
public class GLCMFeatureExtraction {
    ...
    public GLCMFeatureExtraction(File image2, int grayLevel) throws IOException {
        //this.image = ImageIO.read(image);
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inPreferredConfig = Bitmap.Config.ARGB_8888;
        this.image = BitmapFactory.decodeFile(String.valueOf(image2), options);
        //BufferedInputStream reader=new BufferedInputStream(new FileInputStream(image2));
        System.out.println("ahahahah");
        System.out.println("ihihihihi"+image2);
    ...
    private void createGrayLeveledMatrix() {
        for (int i = 0; i < image.getWidth(); i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                ...
                //bitmap rgb = new Color(image.getRGB(i, j));
//                int newR = rgb.getRed();
